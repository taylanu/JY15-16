...
public class Pixoku extends JPanel implements MouseListener, MouseMotionListener
{
 ...
 //post:  returns an int array that is an inverted image of the argument grid - 0's become 1's and 1's become 0's
 //       given 1 0 0, returns 0 1 1
 //             0 1 1          1 0 0
   private static int [][] invert(int [][] grid)
   {
      int[][] inverted = new int[grid.length][grid[0].length];
 ...
 //*****COMPLETE THIS METHOD********
 //pre:   grid is a nonempty array comprised of 0s and 1s
 //post:  returns an int array that is the horizontal mirror image of the argument grid
 //       given 1 0 0, returns 0 0 1
 //             0 1 1          1 1 0
   private static int[][] mirrorFlip(int[][] grid)
   {
      int[][] flip = new int[grid.length][grid[0].length];
      for(int i = 0; i < grid.length; i++)
      {
         for(int j = 0; j < grid[0].length; j++)
         {
            flip[i][j] = grid[i][grid[0].length - 1 - j];
         }
      }
      return flip;
...
//post: returns true if guess and board have black pixels (value 1) in the same location.  Ignores values 2 and 3
//      computes the score, with 1 point for each pixel, (minus the number of errors * 2), plus the (number of unused hints *2)
   private static boolean checkForWin()
   {
      for(int i = 0; i < board.length; i++)
      {
         ...
         {
            if((board[i][j] == 1 && (guess[i][j] != 1 && guess[i][j] != -1)) || (board[i][j] == 0 && (guess[i][j] != 0 && guess[i][j] != 2 && guess[i][j] != 3)))
            {
               return false;
            }
            else
...
//post:  loads in a file with a theme, returns false if it had to pick a random board
   public static boolean createThemedBoard(String forceFile)
   {
      ...
      if(filename!=null)
      {
         ...
         {
            if(Math.random() < .5)	//50% of the time, make the image its horizontal mirror image for more variety
            {
...
//post:  play the randomly selected scale and tone adter the player wins
   ...
   public void userCommand(int key)
   {
      ...
      else if(key == java.awt.event.KeyEvent.VK_ADD || key == java.awt.event.KeyEvent.VK_PLUS)	
      {
         if(paintMode)
         {
            if(guess.length < 16)
      ...
      else if(key == java.awt.event.KeyEvent.VK_SUBTRACT || key == java.awt.event.KeyEvent.VK_MINUS)	
      {
         if(paintMode)
         {
            if(guess.length > 3)
