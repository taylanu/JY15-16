...
public class Display extends TanksPanel {
    ...
    public static void showBoard(Graphics g) {
        ...
        if (sky.getImageLoadStatus() == MediaTracker.COMPLETE)            //image loaded properly
            ...
            if (roundOver()) {    //sky is illuminated when a player is on fire
                if (Math.random() < .5)
            ...
            for (int c = 0; c < board[0].length; c++) {
                if (board[r][c] == true) {
                    ...
                    {  //if the ground tile does not have slope images, just use the flat one (index 0)
                        ...
                        if (roundOver() && sky.getImageLoadStatus() == MediaTracker.COMPLETE) {    //sky is illuminated when a player is on fire
                            if (Math.random() < .5)
        ...
        for (Shell shot : shots) {
            ...
            if (shot.getX() > 0 && shot.getX() < screenWidth && shot.getY(screenHeight) > 0 && shot.getY(screenHeight) < screenHeight) {
                ...
                else if (shot.getType().equals("shell"))
        ...
        for (int i = 0; i < explosions.size(); i++)                                //draw any explosions on the screen
        {
            ...
            int eX = curr.getX();
            int eY = curr.getY();
            if (eX < 0 || eY < 0 || eX > screenWidth || eY > screenHeight || curr.getAnimationIndex() >= explosionImages[0].length - 1) {//remove out-of-bounds or expired explosions
                explosions.remove(i);
                i--;
            } else {
