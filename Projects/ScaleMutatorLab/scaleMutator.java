   ...
   public static int[] makePalindrome(int[]list)
   {
     //*********WRITE THIS METHOD***************
	   int[] pdrome = new int[(list.length * 2) - 1];
	      ...
	      for(int i = list.length; i < pdrome.length; i++){
	         pdrome[i] = pdrome[pdrome.length-1-i];
	      }
      return pdrome;//temporary statement to keep us compiling
     //*****************************************
   }
...
//needs to work with an array of any size
   public static int[] mixWithNote(int[] list, int note)
   {
     //*********WRITE THIS METHOD***************
      ...
      return notes; 	//temporary statement to keep us compiling
     //*****************************************
   }
