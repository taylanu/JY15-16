            ...
            if(Math.abs(curr.getMoveIncrX()) >= SIZE || Math.abs(curr.getMoveIncrY()) >= SIZE)
            {
               ...
               if(lastInput != -1)
               {
                  if(lastInput==KeyEvent.VK_UP && curr.getRow()>0 && board[curr.getRow()-1][curr.getCol()]!=2)
                     ...
                     if(lastInput==KeyEvent.VK_DOWN && curr.getRow()<board.length-1 && board[curr.getRow()+1][curr.getCol()]!=2)
                        curr.setDirection(DOWN);
                     else
                        if(lastInput==KeyEvent.VK_LEFT && curr.getCol()>0 && board[curr.getRow()][curr.getCol()-1]!=2)
                           curr.setDirection(LEFT); 
                        else 
                           if(lastInput==KeyEvent.VK_RIGHT && curr.getCol()<board[0].length-1 && board[curr.getRow()][curr.getCol()+1]!=2)
                              curr.setDirection(RIGHT);
               }        
            }
            else
