...
public class MyGridExample extends JPanel {
    public static final int BULLETLIMIT = 10;    //total # bullets that can be fired on the screen at one time
    ...
    public void processUserInput(int k) {
        ...
        if (k == KeyEvent.VK_W || k == KeyEvent.VK_A || k == KeyEvent.VK_S || k == KeyEvent.VK_D)    //we are shooting with W, A, S or D
        {
            Bullet temp = null;
            if (k == KeyEvent.VK_W) {
                temp = new Bullet("UP", curr.getX(), curr.getY(), bulletImages, SPEED);
                temp.setDirection(UP);
            } else if (k == KeyEvent.VK_D) {
                temp = new Bullet("RIGHT", curr.getX(), curr.getY(), bulletImages, SPEED);
                temp.setDirection(RIGHT);
            } else if (k == KeyEvent.VK_S) {
                temp = new Bullet("DONW", curr.getX(), curr.getY(), bulletImages, SPEED);
                temp.setDirection(DOWN);
            } else if (k == KeyEvent.VK_A) {
                temp = new Bullet("LEFT", curr.getX(), curr.getY(), bulletImages, SPEED);
                temp.setDirection(LEFT);
            }
            if (temp != null)
                bullets.add(temp);
            if (bullets.size() > BULLETLIMIT)    //remove earliest fired bullet if we have more than the bullet limit
                bullets.remove(0);
            repaint();
            return;
        }
