...
public class AIplayer extends Player {
    ...
    public void AImove(Player p, int[][] b, int pR, int pC, int pwR, int pwC) {
        ...
        if (dir == KeyEvent.VK_UP && ((row > 0 && SnakePanel.blocked(row - 1, col)) || (row == 0 && SnakePanel.blocked(board.length - 1, col)))) {//moving up and front is blocked, try to move left or right
            if (Math.random() < .5) {
        ...
        } else if (dir == KeyEvent.VK_DOWN && ((row < board.length - 1 && SnakePanel.blocked(row + 1, col)) || (row == board.length - 1 && SnakePanel.blocked(0, col)))) {//moving down and front is blocked, try to move left or right
            if (Math.random() < .5) {
                if ((col < board[0].length - 1 && !SnakePanel.blocked(row, col + 1)) || (col == board[0].length - 1 && !SnakePanel.blocked(row, 0)))
                    dir = KeyEvent.VK_RIGHT;
                else
        ...
        } else if (dir == KeyEvent.VK_RIGHT && ((col < board[0].length - 1 && SnakePanel.blocked(row, col + 1)) || (col == board[0].length - 1 && SnakePanel.blocked(row, 0)))) {//moving right and front is blocked, try to move up or down
            if (Math.random() < .5) {
        ...
        } else if (dir == KeyEvent.VK_LEFT && ((col > 0 && SnakePanel.blocked(row, col - 1)) || (col == 0 && SnakePanel.blocked(row, board[0].length - 1)))) {//moving left and front is blocked, try to move up or down
            if (Math.random() < .5) {
                if ((row < board.length - 1 && !SnakePanel.blocked(row + 1, col)) || (row == board.length - 1 && !SnakePanel.blocked(0, col)))
                    dir = KeyEvent.VK_DOWN;
                else
        ...
        //*******************turn to get powerups (priority 3)******************
        else if ((dir == KeyEvent.VK_UP || dir == KeyEvent.VK_DOWN) && (pelletToRight(false) || pelletToLeft(false))) {//if there is a grow pellet to our right and left, get it
            if (pelletToRight(false))
                dir = KeyEvent.VK_RIGHT;
            else if (pelletToLeft(false))
                dir = KeyEvent.VK_LEFT;
        } else if ((dir == KeyEvent.VK_LEFT || dir == KeyEvent.VK_RIGHT) && (pelletAbove(false) || pelletBelow(false))) {//if there is a pellet above or below us, get it
            if (pelletAbove(false))
                dir = KeyEvent.VK_UP;
            else if (pelletBelow(false))
                dir = KeyEvent.VK_DOWN;
        } else if ((dir == KeyEvent.VK_UP || dir == KeyEvent.VK_DOWN) && (pelletToRight(true) || pelletToLeft(true))) {    //if there is a powerup to either side, get it
            if (pelletToRight(true))
                dir = KeyEvent.VK_RIGHT;
            else if (pelletToLeft(true))
                dir = KeyEvent.VK_LEFT;
        } else if ((dir == KeyEvent.VK_LEFT || dir == KeyEvent.VK_RIGHT) && (pelletAbove(true) || pelletBelow(true))) {    //if there is a powerup above or below, get it
            if (pelletAbove(true))
                dir = KeyEvent.VK_UP;
        ...
        //************************seek powerups (priority 4)***********************
            ...
            dir = KeyEvent.VK_LEFT;
            //************************make random turns (priority 5)*********************
        else if (Math.random() < unpredictability) {    //make some random moves
            if (dir == KeyEvent.VK_UP || dir == KeyEvent.VK_DOWN) {
                if (Math.random() < .5 && col < board[0].length - 1 && !SnakePanel.blocked(row, col + 1)) {//make sure we don't try to make 4 turns in the same direction in succession
                    if ((dir == KeyEvent.VK_UP && numRights < 4) || (dir == KeyEvent.VK_DOWN && numLefts < 4))
                        dir = KeyEvent.VK_RIGHT;
                } else if (col > 0 && !SnakePanel.blocked(row, col - 1)) {//make sure we don't try to make 4 turns in the same direction in succession
                    if ((dir == KeyEvent.VK_DOWN && numRights < 4) || (dir == KeyEvent.VK_UP && numLefts < 4))
                        dir = KeyEvent.VK_LEFT;
                }
            } else if (dir == KeyEvent.VK_LEFT || dir == KeyEvent.VK_RIGHT) {
                if (Math.random() < .5 && row > 0 && !SnakePanel.blocked(row - 1, col)) {//make sure we don't try to make 4 turns in the same direction in succession
                    if ((dir == KeyEvent.VK_LEFT && numRights < 4) || (dir == KeyEvent.VK_RIGHT && numLefts < 4))
                        dir = KeyEvent.VK_UP;
                } else if (row < board.length - 1 && !SnakePanel.blocked(row + 1, col)) {//make sure we don't try to make 4 turns in the same direction in succession
