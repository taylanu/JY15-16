...
public class Utilities extends Pixoku        //has access to items in Pixoku.java
{
    ...
    public static void drawPixelCount(Graphics g) {
        int x = (maxRowCount * size), y = 0;        //upper left corner location of where image will bedrawn
    ...
    public static void showBoard(Graphics g) {
        ...
        for (int r = 0; r < guess.length; r++) {
            ...
            for (int c = 0; c < guess[0].length; c++) {
                ...
                } else if (solved)    //show the original board with black and white pixels without borders
                {
                    ...
                    if (guess[r][c] == 0)                    //unmarked spaces
                        g.drawImage(BLANK.getImage(), x, y, size, size, null);
                    else if (guess[r][c] == 1)                //painted spaces
                        g.drawImage(PAINTED.getImage(), x, y, size, size, null);
                    else if (guess[r][c] == 2)            //spaces marked with an X
                        g.drawImage(EXOUT.getImage(), x, y, size, size, null);
                    else if (guess[r][c] == 3)        //spaces marked by the computer as a mistake
        ...
        if (!paintMode && solved && solutionImage != null && solutionImage.length() > 0 && !solutionImage.equals("none")) {    //draw a picture on top of the board
        ...
        for (int r = 0; r < guess.length; r++)
            for (int c = 0; c < guess[0].length; c++) {
                if (Math.abs(guess[r][c]) == 1 && board[r][c] != 1)//player painted a cell that is not in the solution
                {
                    ...
                    numPixels++;
                } else if (guess[r][c] == 2 && board[r][c] == 1)        //player exed out a cell that is in the solution
                {
    ...
    public static void helpPlayer() {
        ...
        if (hint > 0) {
            ...
            for (int r = 0; r < guess.length; r++)
                for (int c = 0; c < guess[0].length; c++) {
                    if (Math.abs(guess[r][c]) != 1 && board[r][c] == 1) {//store the row and col of the unguessed spot so we can choose some random ones
                        rows.add(r);
                        cols.add(c);
                    }
                }
