...
//intervals of scales						Major							minor						Blues				Harmonic minor
    ...
    public Pixoku()                            //constructor
    {
        ...
        else {        //start with a themed board read from a randomly picked file
...
//			grid is to remain unchanged
    public static int[][] invert(int[][] grid) {
        int[][] invGrid = new int[grid.length][grid[0].length];
...
//			grid is to remain unchanged
    public static int[][] mirrorFlip(int[][] grid) {
        int[][] flipGrid = new int[grid.length][grid[0].length];
        for (int i = 0; i < grid.length; i++)
            for (int j = 0; j < grid[0].length; j++)
                flipGrid[i][j] = grid[i][grid[0].length - 1 - j];
        return flipGrid;    //temporary statement to keep things compiling
    }
...
//      the data field hint will have the correct value for the number of unused hints when the method is called
    private static boolean checkForWin() {
        ...
        return true;    //temporary statement to keep things compiling
    }
...
//			calls appropriate method depending on key hit
    public void userCommand(int key) {
            ...
            if ((key >= 'A' && key <= 'Z') || (key >= '0' && key <= '9') || key == java.awt.event.KeyEvent.VK_SPACE) {
                if ((whichText[TEXT_THEME] || whichText[TEXT_NAME] || whichText[TEXT_FILE]) && key == java.awt.event.KeyEvent.VK_SPACE) {
                    ...
                    if (whichText[TEXT_NAME])    //only 3 characters for initials
                    {
                        if (message.length() < 3)
                            message += (char) (key);
                    } else if (message.length() < 30)    //max 30 characters
                        message += (char) (key);
                }
        ...
        else if (key == java.awt.event.KeyEvent.VK_E && !solved)                //check for errors
        {
            ...
            else if (solved && scoreIndex >= 0 && scoreIndex < list.size()) {                                                                    //enter high score initials
                textInput = true;
        ...
        } else if (key == java.awt.event.KeyEvent.VK_X)                //toggle whether or not the space is ex-ed out or not
        {
            ...
            } else if (solved)                            //reset high scores
            {
                ...
                updateScores = true;
            } else if (guess[playerR][playerC] != -1 && guess[playerR][playerC] != 3 && !solved) {//you can't alter what the computer places - hint or showing a mistake
        ...
        {
            if (guess[playerR][playerC] != -1 && guess[playerR][playerC] != 3 && !solved) {//you can't alter what the computer places - hint or showing a mistake
